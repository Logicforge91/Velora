...
class ShipmentOperationsService
{
    ...
    public function update(Shipment $shipment, array $data): Shipment
    {
        DB::transaction(function () use ($shipment, $data): void {
            $shipment->update([...$data, 'shipped_at' => in_array($data['status'], ['shipped', 'in_transit', 'delivered'], true) ? ($shipment->shipped_at ?? now()) : null, 'delivered_at' => $data['status'] === 'delivered' ? now() : null]);
            $orderStatus = match ($data['status']) {
                'shipped', 'in_transit' => Order::STATUS_SHIPPED, 'delivered' => Order::STATUS_DELIVERED, 'returned' => Order::STATUS_CANCELLED, default => (string) $shipment->order()->value('status')
            };
