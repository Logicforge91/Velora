...
export default function CategoryForm({
    ...
    return (
        <AdminLayout
            title={exists ? 'Edit Category' : 'Create Category'}
            breadcrumb={`Categories / ${exists ? 'Edit' : 'Create'}`}
        >
            <CatalogueForm
                kind="category"
                item={category}
                ...
                submitUrl={
                    exists
                        ? categoriesRoutes.update.url(category.id)
                        : categoriesRoutes.store.url()
                }
                method={exists ? 'put' : 'post'}
                cancelUrl={categoriesRoutes.index.url()}
            />
        </AdminLayout>
    );
}
